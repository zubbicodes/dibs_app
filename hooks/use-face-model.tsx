...
let nativeTflite: any;
if (Platform.OS !== 'web') {
  nativeTflite = require('@/lib/native-tflite');
}
...
export function FaceModelProvider({ children }: { children: ReactNode }) {
  ...
  const loadFromDisk = async () => {
    if (Platform.OS !== 'web') {
      return nativeTflite.loadTensorflowModel({ url: MODEL_PATH }, []);
    }
  };
  ...
  const downloadAndLoadModel = async () => {
    ...
    try {
      ...
      if (Platform.OS === 'web') {
        // Web uses face-api.js which loads its own models in the modal
        setState('loaded');
        return;
      }
